...
class PolicyTest(ControlPlaneBaseTest):
    ...
    def check_constraints(self, send_count, recv_count, time_delta_ms, rx_pps):
        ...
        if self.has_trap:
            ...
            assert self.PPS_LIMIT_MIN <= rx_pps <= self.PPS_LIMIT_MAX, "Copp policer constraint check failed, Actual PPS: {} " \
                    "Expected PPS range: {} - {}".format(rx_pps, self.PPS_LIMIT_MIN, self.PPS_LIMIT_MAX)
        else:
...
class BGPTest(PolicyTest):
    ...
    def check_constraints(self, send_count, recv_count, time_delta_ms, rx_pps):
        ...
        if self.has_trap:
            ...
            assert self.DEFAULT_PPS_LIMIT_MIN <= rx_pps <= self.DEFAULT_PPS_LIMIT_MAX, "Copp policer constraint check failed, " \
                "Actual PPS: {} Expected PPS range: {} - {}".format(rx_pps, self.DEFAULT_PPS_LIMIT_MIN, self.DEFAULT_PPS_LIMIT_MAX)
